...
import valkyrie/internal/client.{execute, execute_blocking}
import valkyrie/internal/command/list as command
  ...
  |> result.map(fn(value) {
    case value {
      [protocol.Array(array)] ->
        ...
        |> list.try_map(fn(kv) {
          case kv {
            [protocol.BulkString(key), protocol.BulkString(value)] ->
              Ok(#(key, value))
            _ -> Error(error.RESPError)
          }
        })
